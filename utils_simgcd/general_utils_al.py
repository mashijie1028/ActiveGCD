...
def init_experiment(args, runner_name=None, exp_id=None):
    ...
    now = args.base_ckpts_date
    if args.base_exp_id is not None:
        #now = now + '_' + args.base_exp_id
        now = args.base_exp_id + '_' + now
    now = now + '_query' + str(args.num_query)   # NOTE!!! add #query to log folder

    # AL path dir
    log_dir = os.path.join(root_dir, now, args.strategy)
